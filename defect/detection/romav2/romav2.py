...
class RoMaV2(nn.Module):
    ...
    def __init__(self, cfg: Cfg | None = None, weights_path: str | Path | None = None):
        super().__init__()
        if cfg is None:
            # default
            cfg = RoMaV2.Cfg()
        ...
        if weights_path is None:
            raise ValueError("weights_path is required by the bundled offline RoMaV2 runtime")
        weights = torch.load(
            Path(weights_path), map_location=device, weights_only=True
        )
