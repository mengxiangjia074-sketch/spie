...
class Descriptor:
    ...
    def __new__(cls, cfg: Cfg) -> nn.Module:
        ...
        match cfg.name:
            case "dinov3_vitl16":
                normalizer = imagenet
                descriptor_model: nn.Module = dinov3_vitl16(
                    pretrained=cfg.weights_path is not None,
                    weights=cfg.weights_path,
                ).to(device)
                layers = _get_layers(cfg.layer_idx, descriptor_model)
                return partial_wrap(
                    descriptor_model,
                    normalizer=normalizer,
                    patch_size=16,
                    func=partial(descriptor_model.get_intermediate_layers, n=layers),
                )
            case "dinov2_vitl14":
                raise NotImplementedError(
                    "The bundled RoMaV2 runtime supports the local DINOv3 descriptor only"
                )
            case "mum_vitl16":
